 ...
 *
 * This version of the messaging resource blocks when POSTing a new message
...
public class BlockingPostChatResource {
	...
	private static final ExecutorService QUEUE_EXECUTOR = Executors
			.newCachedThreadPool(new ThreadFactoryBuilder().setNameFormat("blocking-post-chat-resource-executor-%d")
	...
	static {
		PropertyConfigurator
				.configure(BlockingPostChatResource.class.getClassLoader().getResource("log4j.properties"));
	}
	...
	@POST
	public void postMessage(@Suspended final AsyncResponse ar, final String inputMessage) {
		String message = inputMessage + " Pradip";
			...
			public void run() {
				try {
					...
					System.out.println(String.format(
							"POSTed message {%s} successfully queued. Resuming POST with context {%s} on thread {%ss}.",
							message, ar.toString(), Thread.currentThread().getName()));

					ar.resume(message + " :: " + POST_NOTIFICATION_RESPONSE);
				} catch (InterruptedException ex) {
					LOGGER.log(Level.SEVERE, "Waiting for a queueing a message '" + message + "' has been interrupted.",
							ex);
